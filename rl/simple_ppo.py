...
def train_ppo(env, batch_size=5000, num_epochs=1000, gamma=0.99, lr=1e-3, policy_epochs=80, value_epochs=80, clip_eps=0.2):
    ...
    old_policy_net = PolicyNetwork(obs_dim, action_dim)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            old_log_probs = old_policy_net.get_log_probs(trajectory['observations'], trajectory['actions'])
        ...
        # update old policy with the current policy
        old_policy_net.load_state_dict(policy_net.state_dict())
